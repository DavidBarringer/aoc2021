...
(defun bench (&optional day func-name &rest args)
  ...
  (cond ((null func-name) (print-bench (format nil "Day ~D (all parts)" (day-number (eval loaded-day)))
																			 (+ (bench-fraction (format nil "Day ~D parsing input" (day-number (eval loaded-day))) 'parse-input (day-input-file (eval loaded-day)))
																					(bench-fraction (format nil "Day ~D part a" (day-number (eval loaded-day))) 'part-a (day-parsed-input (eval loaded-day)))
																					(bench-fraction (format nil "Day ~D part b" (day-number (eval loaded-day))) 'part-b (day-parsed-input (eval loaded-day))))))
        ((eq func-name 'a) (bench-fraction (format nil "Day ~D part a" (day-number (eval loaded-day))) 'part-a (day-parsed-input (eval loaded-day)))
				 (format t ""))
        ((eq func-name 'part-a) (bench-fraction (format nil "Day ~D part a" (day-number (eval loaded-day))) 'part-a (day-parsed-input (eval loaded-day)))
				 (format t ""))
        ((eq func-name 'b) (bench-fraction (format nil "Day ~D part b" (day-number (eval loaded-day))) 'part-b (day-parsed-input (eval loaded-day)))
				 (format t ""))
        ((eq func-name 'part-b) (bench-fraction (format nil "Day ~D part b" (day-number (eval loaded-day))) 'part-b (day-parsed-input (eval loaded-day)))
				 (format t ""))
        ((eq func-name 'parse) (bench-fraction (format nil "Day ~D parsing input" (day-number (eval loaded-day))) 'parse-input (day-input-file (eval loaded-day)))
				 (format t ""))
        ((eq func-name 'parse-input) (bench-fraction (format nil "Day ~D parsing input" (day-number (eval loaded-day))) 'parse-input (day-input-file (eval loaded-day)))
				 (format t ""))
        (t (bench-fraction (format nil "Day ~D function ~A" (day-number (eval loaded-day)) func-name args))
				(format t ""))))
